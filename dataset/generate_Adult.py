...
def download_adult_dataset(data_path):
    ...
    if not os.path.exists(train_file):
        ...
        os.system(f"wget {train_url} -O {train_file}")
    
    if not os.path.exists(test_file):
        print("Downloading Adult test data...")
        os.system(f"wget {test_url} -O {test_file}")
...
def preprocess_adult_data(train_file, test_file):
    ...
    # Load data
    train_data = pd.read_csv(train_file, names=column_names, sep=',\s*', engine='python', na_values='?')
    test_data = pd.read_csv(test_file, names=column_names, sep=',\s*', engine='python', 
