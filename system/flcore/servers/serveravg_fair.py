...
class FedAvg_Fair(FedAvg):
    """FedAvg with fairness metrics tracking (EOD, AccGap, AccStd, AccWorst).

    Inherits the standard FedAvg training loop unchanged and only extends
    evaluate() and save_results() to record fairness metrics.  Used as a
    baseline that shows the fairness cost of vanilla averaging.
    """
